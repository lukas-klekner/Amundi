...
def transform(fileRequest, bucketResponse, keyResponse):
        ...
        if x1 == True:
            ...
            df['SecurityISIN'] = df['SecurityISIN'].apply(lambda x: x.replace('USD','').replace('AUD','').replace('SGD','').replace('JPY','').replace('HUF','').replace('CHF','').replace('EUR','').replace('GBP','').replace('CZK','').replace('SWAP','').replace('OACT','').replace('OVCT','').replace('-',''))                
